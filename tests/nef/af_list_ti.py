...
def list_traffic_influence_subscriptions(nef_url, af_id):
    endpoint = f"{nef_url}/3gpp-traffic-influence/v1/{af_id}/subscriptions"
    response = requests.get(endpoint)

    if response.status_code == 200:
        print("Inscrições ativas:")
        print(response.json())
    else:
        print(f"Erro ao listar inscrições: {response.status_code}")
        print(response.text)

if __name__ == '__main__':
    # Exemplo de uso
    list_traffic_influence_subscriptions(NEF_BASE_URL, AF_ID)
